...
//add gaojt [Robust-Query-Optimization] 2019-02-15:e
...
//add gaojt [Robust-Query-Optimization] 2019-02-15:b
static void generate_hollow_range(Relation onerel, VacAttrStats *stats);
static int  compare_hr(const void *a, const void *b);
...
//add gaojt [Robust-Query-Optimization] 2019-02-15:b
/*
 * Construct hash table for values of one column according to StorePreparedStatement
 *
 */

/*
 * For generating the hollow range, we need fetching all rows of a relation,
 * so for avoiding affecting the normal procedure of building statistics through
 * sampling, we not only use the sampled blocks, but also record the miss-fetched blocks and
 * proceed the procedure of generating hollow range with these blocks later.
 *
 */
/*
 * record the block number of missed blocks when generating statistics using sampling method
 */
#define DEFAULT_MAX_ROW_NUM 200000000
#define MAX_UINT64 18446744073709551615L
#define MIN_INTERVAL_STEP 100
#define DEFAULT_MAX_HR_NUMBER 1000000
typedef struct HollowRange
{
    int   l_bi;//index of left bound
    float v_im;//the value fo importance
}HollowRange;
//Descending Order
static int
compare_hr(const void *a, const void *b)
{
    HollowRange *l_hr = (HollowRange*)a;
    HollowRange *r_hr = (HollowRange*)b;

    if (r_hr->v_im < l_hr->v_im)
    {
        return -1;
    }
    else if (r_hr->v_im > l_hr->v_im)
    {
        return 1;
    }
...
static void
print_array(int *a, int length, const char *comment)
{
    char     res[1024000];
    char     tmp[256];
    int      i;
...
int slot_index_hr = 0;
static void
generate_hollow_range(Relation onerel,VacAttrStats *stats)
{
    double		deadrows = 0;	/* # dead rows seen */
    BlockNumber totalblocks;
    TransactionId OldestXmin;
    BlockSamplerData bs;
    Datum       min = MAX_UINT64-1;
    Datum       max = 0;
    Bitmapset   *c_values = NULL;
    ...
    bs.N = totalblocks;			/* measured table size */
    bs.t = 0;					/* blocks scanned so far */
    elog(LOG, "begin palloc for c_values");
    c_values = (Bitmapset *) palloc(DEFAULT_MAX_ROW_NUM*sizeof(Bitmapset));
    elog(LOG, "begin build c_values,size of c_values=%d, c_values->nwords=%d",
         DEFAULT_MAX_ROW_NUM*sizeof(Bitmapset), c_values->nwords);
    //process all blocks
    while (bs.t < bs.N)
    {
        BlockNumber targblock = bs.t++;
        Buffer		targbuffer;
        Page		targpage;
...
//        vacuum_delay_point();
//        elog(LOG, "the %dth block is analyzed", targblock);
        ...
        for (targoffset = FirstOffsetNumber; targoffset <= maxoffset; targoffset++)
        {
            ...
            value = heap_getattr(heap_copytuple(&targtuple), (stats->tupattnum), (stats->tupDesc), &isnull);

            if(DatumGetInt32(value)>=0)
            {
                bms_add_member(c_values, value);
                if (min > value)
                {
                    min = value;
                }
            ...
            {
                elog(LOG,"negative value=[%d]", DatumGetInt32(value));
            }

        }
        if (targblock%1000 == 0)
        {
            elog(LOG, "after analyzing %d block, the number of distinct values is:%d",targblock, c_values->nwords);
        }

        UnlockReleaseBuffer(targbuffer);

    }
    elog(LOG, "build c_values finished");
    //generate hollow range, only for column with type int32
//    Datum str = (Datum)("abc");
//    elog(LOG, "orig-int=%d, orig-str=%s, modi-int1=%d, modi-int2=%d, modi-str=%s",
//         DatumGetInt32(min), DatumGetCString(str), DatumGetInt32(min+1), DatumGetInt32(++min), DatumGetCString(str+2));
    int i = 0;
    int j = 0;
    int lower_bound = 0;
    int upper_bound = 0;
    int _min = DatumGetInt32(min);
    int _max = DatumGetInt32(max);
    int hr_bounds[DEFAULT_MAX_HR_NUMBER];

    memset(hr_bounds,0xFFFF,sizeof(hr_bounds));
    elog(LOG, "_min=%d, _max=%d, ",_min,_max);
    // The first hr is [min,max], used to change the infinite bound value of column range to non-infinite value
    hr_bounds[j++] = _min;
    hr_bounds[j++] = _max;
    int counter = 0;
    for (i = _min+1;i <= _max;i ++)
    {
        if (!bms_is_member((Datum)(i), c_values))
        {
            lower_bound = i;
            //search upper bound
            if (!bms_is_member((Datum)(i+MIN_INTERVAL_STEP), c_values))
            {
                int tmp_upper = i+MIN_INTERVAL_STEP;
                int _i = 0;
                int rand_n = 0;
                srand(time(NULL));
                //pre-probe to find the first existing member
                for (_i=1;_i < 20;_i ++)
                {
                    rand_n = rand()%100;
                    //if we found one member, then we think [i,i+MIN_INTERVAL_STEP]
                    //is a low-worth interval, we will explore from i+MIN_INTERVAL_STEP
                    if (bms_is_member((Datum)(tmp_upper-rand_n), c_values))
                    {
                        i = i+MIN_INTERVAL_STEP;
                        break;
                    }
                }
                //bad luck, but we should control the risk
                if (_i==20 && !bms_is_member((Datum)(i+1.5*MIN_INTERVAL_STEP), c_values))
                {
                    int k;
                    for (k = i;k <=_max;k ++)
                    {
                        if (bms_is_member((Datum)k,c_values))
                        {
                            if (counter >= MIN_INTERVAL_STEP)
                            {
                                upper_bound = k-1;
                                hr_bounds[j++] = lower_bound;
                                hr_bounds[j++] = upper_bound;
                                i = k;
                                elog(LOG, "Exploring one HR:lower_bound=%d,upper_bound=%d",lower_bound,upper_bound);
                            }
                            else
                            {
                                i=k;
            ...
            {
                i = i+MIN_INTERVAL_STEP+1;
            }
        }

    }
    pfree(c_values);
    elog(LOG, "finish building HRs and freeing c_values");
    /**********************DEBUG-b***********************/
//    char res[DEFAULT_MAX_HR_NUMBER*20];
//    char tmp[64];
//    memset(tmp,0,sizeof(tmp));
//    memset(res,0,sizeof(res));
//    int m;

//    snprintf(tmp,sizeof(tmp),"%d",hr_bounds[0]);
//    strcat(res,tmp);
//    for (m=1;m<DEFAULT_MAX_HR_NUMBER;m++)
//    {
//        if (hr_bounds[m] != -1)
//        {
//            snprintf(tmp,sizeof(tmp),",%d",hr_bounds[m]);
//            strcat(res,tmp);
//        }
//    }
//    elog(LOG, "hr_bounds=[%s]",res);
    /**********************DEBUG-e***********************/

    //update pg_statistics
    int x;
    int num_hr = 0;
    for (x = 0;x < DEFAULT_MAX_HR_NUMBER;x ++)
    {
        if (hr_bounds[x] != -1)
        {
            num_hr ++;
    ...
    print_array(hr_bounds,num_hr,"total res:");
#if 0
//    print_array(hr_bounds,num_hr,"hr_bounds:");
    elog(LOG, "num_hr=%d, slot_index_hr=%d", num_hr, slot_index_hr);
    /*
     * For reducing the count of unnecessary HRs, we evaluate each generated HR,
     * and rank them. Then we pick the top-k HRs.
     *
     */
#define bound 50
#define p_r_hrs 0.7
#define p_f_hrs 0.2
#define p_t_hrs 0.2
#define p_m_hrs (1.0-p_f_hrs-p_t_hrs)
#define random_threshold 100

    int   res_hr[2*bound+1]  = {0};
    int   f_hr_num;
    int   m_hr_num;
    int   t_hr_num;
    if (num_hr/2 <= bound)
    {
        for (i = 0;i < num_hr;i ++)
        {
            res_hr[i] = hr_bounds[i];
        }
    }
    else
    {
        f_hr_num = bound*p_f_hrs;
        m_hr_num = bound*p_m_hrs;
        t_hr_num = bound*p_t_hrs;
        elog(LOG, "f_hr_num=%d, m_hr_num=%d, t_hr_num=%d", f_hr_num, m_hr_num, t_hr_num);
        /*
         * Fill res with front HRs, middel HRs, and tail HRs
         */
        for (i = 0;i < 2*f_hr_num;i ++)
        {
            res_hr[i] = hr_bounds[i];
        }
        for (i = 0;i < 2*t_hr_num;i ++)
        {
            res_hr[2*bound-2*t_hr_num+i] = hr_bounds[num_hr-2*t_hr_num+i];
        }
        print_array(res_hr,2*bound+1,"front+tail res:");

        int j = 0;
        float mid_value = (_max-_min)/2;
        float location_cost;
        HollowRange hr[num_hr-2*f_hr_num-2*t_hr_num+1];//record the middle hrs
        int   ranked_hr_num = 0;
        for (i = 2*f_hr_num;i < num_hr-2*t_hr_num+1;i ++)
        {
            hr[j].l_bi = i;
            if (hr_bounds[i+1] < mid_value)
            {
                location_cost = mid_value - hr_bounds[i+1];
            }
            else if (hr_bounds[i] <= mid_value && mid_value <= hr_bounds[i+1])
            {
                location_cost = 0;
            }
            else if (hr_bounds[i] > mid_value)
            {
                location_cost = hr_bounds[i] - mid_value;
            }
            hr[j].v_im = (hr_bounds[i+1]-hr_bounds[i])+location_cost;
            i ++;
            j ++;
        }
        ranked_hr_num = j;
        qsort(hr, ranked_hr_num, sizeof(HollowRange), compare_hr);

        /*
         * It does not need to randomly pick the ranked HRs
         *
         */
        if (num_hr/2 <= random_threshold)
        {

            elog(LOG, "num of ranked HollowRange=%d",ranked_hr_num);
            int x = 0;
            for (i = 0;i < ranked_hr_num;i ++)
            {
                res_hr[2*f_hr_num+x]   = hr_bounds[hr[i].l_bi];
                res_hr[2*f_hr_num+x+1] = hr_bounds[hr[i].l_bi+1];
                x = x+2;
                if (x == 2*m_hr_num)
                {
                    break;
            ...
            num_hr = 2*bound;
            print_array(res_hr,2*bound+1,"front+middle+tail res:");
        }
        /*
         * If the number of HRs is relatively large, we need to first
         * rank all middle HRs, then randomly pick the ranked HRs to keep
         * the discrete charactor.
         *
         */
        else
        {
            int picked_HRs_num = p_r_hrs * ranked_hr_num;
            int rand_index;
            elog(LOG, "picked_HRs_num=%d, ranked_hr_num=%d", picked_HRs_num, ranked_hr_num);
            int x = 0;
            for (i = 0;i < m_hr_num;i ++)
            {
                rand_index = rand()%picked_HRs_num;
                res_hr[2*f_hr_num+x]   = hr_bounds[hr[rand_index].l_bi];
                res_hr[2*f_hr_num+x+1] = hr_bounds[hr[rand_index].l_bi+1];
                x = x + 2;
            }
            num_hr = 2*bound;
            print_array(res_hr,2*bound+1,"front+middle+tail res:");
    ...
    if (num_hr >= 2 && slot_index_hr < 5)
    {
        ...
        hr_values = (Datum *) palloc(num_hr * sizeof(Datum));

        for (y = 0; y < num_hr; y++)
        {
            hr_values[y] = datumCopy((Datum)hr_bounds[y],
                    stats->attrtype->typbyval,
                    stats->attrtype->typlen);
        }
        MemoryContextSwitchTo(old_context);

        stats->stakind[slot_index_hr] = STATISTIC_KIND_BOUNDS_HOLLOW_RANG;
        stats->staop[slot_index_hr] = mystats->eqopr;
        stats->stanumbers[slot_index_hr] = 0;
        stats->numnumbers[slot_index_hr] = 0;
        stats->stavalues[slot_index_hr] = hr_values;
        stats->numvalues[slot_index_hr] = num_hr;
    ...
    else if (slot_index_hr >= 5)
    {
        elog(LOG, "There is no space for hr,slot_index_hr=%d",slot_index_hr);
    }
}
//add gaojt 2019-02-15:e
/*
			   ...
			   int elevel)
{
    ...
    //add gaojt [Robust-Query-Optimization] 2019-02-15:b
    elog(LOG, "targrows=%d, totalrows=%d", targrows, totalrows);
    /*
     * ugly, only for support hollow range.
     * Later, this assignment should be triggered only through hint grammer
     *
     */
    //add gaojt 2019-02-15:e
	/*
	...
	if (numrows > 0)
	{
		...
		for (i = 0; i < attr_cnt; i++)
		{
			...
			stats->compute_stats(stats,
								 ...
								 totalrows);
            //add gaojt [Robust-Query-Optimization] 2019-02-15:b
            generate_hollow_range(onerel, stats);
            //add gaojt 2019-02-15:e
			/*
...
//            if (i == (bs.N-1))
//            {
//                snprintf(tmp,sizeof(tmp),"%d[%d]",i,m_bn[i]);
//            }
//            else
//            {
//                snprintf(tmp,sizeof(tmp),"%d[%d],",i,m_bn[i]);
...
//        }
//        strcat(res, ")");
//        elog(LOG, "distribution of used blocks:%s", res);
//    }
    //add gaojt 2019-02-15:e
					 ...
					 double totalrows)
{
    ...
    //add gaojt [DEBUG] [Robust-Query-Optimization] 2019-02-15:b
//    int x = 1;
//    Datum tmp = (Datum)x;

//    if (bms_is_member(tmp, c_values))
//    {
//        elog(LOG, "[x=%d] is the member of c_values", x);
//    }
    //add gaojt 2019-02-15:e
	...
	if (values_cnt > 0)
	{
        ...
        slot_index_hr = slot_idx;//add gaojt [Robust-Query-Optimization] 2019-02-15
        elog(LOG, "compute_scalar_stats:slot_idx=%d", slot_idx);//add gaojt [DEBUG]
	}
