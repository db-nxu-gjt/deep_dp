...
/*
 * For example, there is a join qual 1.1 = 2.1
 * then, l_lv=1; l_rv=1; r_lv=2; r_rv=1; l_i may equal to 1 and r_i may equal to 3,
 * which are separately monotonously increased
 */
typedef struct JoinQuals
{
    int l_i;//index of left operator
    int l_vt;//var type of left operator
    int l_lv;//table index in the value of left operator
    int l_rv;//attribution index in the value of left operator

    int r_i;//index of right operator
    int r_lv;//table index in the value of right operator
    int r_vt;//var type of right operator
    int r_rv;//attribution index in the value of right operator
}JoinQuals;

typedef struct VarMapIndex
{
    int _i; //index of var
    int v_t;
    int t_i;
...
typedef struct PruneUnit
{
    NodeTag		type;
    bool        is_regressed;
    List        *tci_rs;//the ranges(TcRange) of one prunning unit, corresponding to one transitive closure
}PruneUnit;

//typedef struct TciSubRange
//{
//    Datum        l_b;//lower value of range
//    Datum        u_b;//upper value of range
//    bool         l_i;//indicate the state of lower boundary, true represents '[', false '('
//    bool         u_i;//indicate the state of upper boundary, true represents ']', false ')'
//    bool         l_in;//indicate whether upper value is infinate, true represents yes
//    bool         u_in;//indicate whether lower value is infinate, true represents yes
//    bool         is_legal;//indicate this range whether is legal
//    bool         is_eaten_all;
//    bool         is_eaten_nothing;
//}TciSubRange;

typedef struct TciRange
{
    TCItem       *tci;//(t_i,a_i) used to represent one TciRange
    int          v_t;//value type of this range
    Datum        l_b;//lower value of range
    Datum        u_b;//upper value of range
    bool         l_i;//indicate the state of lower boundary, true represents '[', false '('
    bool         u_i;//indicate the state of upper boundary, true represents ']', false ')'
    ...
    bool         is_eaten_all;
    bool         is_eaten_nothing;
    List         *sub_rs;//sub-ranges
}TciRange;

typedef struct TransitiveClosure
{
    NodeTag		type;
    List*       items; //record information about items in each transitive closure
}TransitiveClosure;

typedef void* SplitRange(void*);

/*
 * for general prunning
 */
typedef struct GpEquivalenceClass
{
    NodeTag		type;
    RangeTblRef* gec;
